...
**Do NOT use for:** Deployments (use CLI), creating resources (use CLI/API), builds/tests (use Bash), file operations (use Write/Edit, or MCP tools on Windows).
...
**Env files:** Use Write/Edit tools (or MCP tools on Windows). Never ask human to create .env manually.
...
| Write .env file | Yes (Write tool or MCP) | YES |
